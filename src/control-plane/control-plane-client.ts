...
import {
  ...
  ControlPlaneProxyResolution,
  ControlPlaneProxyResolver,
  ControlPlaneRequestFunction,
  ControlPlaneRequestOptions,
  ControlPlaneResponse,
  ControlPlaneTransportError,
  ControlPlaneTransportErrorCode,
  CreateControlPlaneClientFromEnvOptions
} from "./control-plane-client-core.js";
export type {
  DeviceAuthRequest,
  DeviceAuthResponse,
  PairClaimRequest,
...
export class ControlPlaneClient {
  ...
  /**
   * Starts a new host pairing session.
   *
   * @param input Pairing-session payload.
   * @returns Pairing-session response body.
   */
  async createPairingSession(input: PairSessionRequest): Promise<PairSessionResponse> {
    return this.post<PairSessionResponse>("pair/sessions", input);
  }
  ...
  /**
   * Proves possession of the device private key for one pairing claim.
   *
   * @param input Pairing proof payload.
   * @returns Proof verification response body.
   */
  async provePairing(input: PairProofRequest): Promise<PairProofResponse> {
    return this.post<PairProofResponse>("pair/prove", input);
  }

  /**
   * Finalizes one pairing flow after the human verification step.
   *
   * @param input Pair confirmation payload.
   * @returns Issued device identity and token set.
   */
  async confirmPairing(input: PairConfirmRequest): Promise<PairConfirmResponse> {
    return this.post<PairConfirmResponse>("pair/confirm", input);
  }

  /**
   * Rotates device credentials and returns a fresh token set.
   *
   * @param input Refresh payload.
   * @returns Refreshed access and refresh credentials.
   */
  async refreshDeviceAccess(
    input: RefreshDeviceAccessRequest
  ): Promise<RefreshDeviceAccessResponse> {
    return this.post<RefreshDeviceAccessResponse>("auth/refresh", input);
  }

  /**
   * Revokes a registered trusted device.
   *
   * @param input Revoke payload.
   * @returns Revocation response body.
   */
  async revokeDevice(input: RevokeDeviceRequest): Promise<RevokeDeviceResponse> {
    return this.post<RevokeDeviceResponse>("devices/revoke", input);
  }
