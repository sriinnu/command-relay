...
import { createRelayProxyServer, normalizeRelayOptions, parseRelayProxyEnv } from "../src/index.js";
...
test("rejects certificate without private key", () => {
    assert.throws(() => normalizeRelayOptions(createRelayOptions({
        upstreamTls: {
            rejectUnauthorized: true,
            cert: "temp-cert",
            key: undefined
...
test("returns contract metadata in stats config", async () => {
    const handle = await createRelayProxyServer(createRelayOptions({
